...
class Builtin(Enum):
    ...
    SIMPLY_DISTR_LOAD = 'simply_supported_distr_load'
...
class Model(mspec.user_class):
    ...
    def from_example(cls, example=Builtin.CANTILEVER.value):
        ...
        elif example == Builtin.SIMPLY_DISTR_LOAD.value:
            return get_example_simply_distr_load()
        else:
...
def get_example_simply_distr_load():
    model = init_examples()
    ...
    beam.add('distr_load', {'from': 'a', 'to': 'c', 'load': [0, 0, -1, 0, 0, 0]})
